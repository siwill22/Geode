...
import type { DeformationLayer } from './deformationInstance';
...
export interface DeformationViewState {
  reconstruction: string;
  layer: DeformationLayer;
  variable: string;
  age: number;
  clipMin: number;
  clipMax: number;
  noDataStyle: NoDataStyle;
  /** UI-only: whether the clip sliders are currently shown in log10 space.
   *  Only meaningful (and only offered) for a sequential Variable whose
   *  range spans enough decades that a linear slider can't usefully reach
   *  the low end -- see rebuildClipControls(). Not read by rendering code;
   *  onClip() always receives physical values regardless. */
  logScale: boolean;
}

export interface DeformationUICallbacks {
  onReconstruction(id: string): void;
  onLayer(layer: DeformationLayer): void;
...
/** Below this ratio, a linear slider already reaches the low end
 *  acceptably -- log mode is for a variable like effective_strain_rate,
 *  whose 99.5th-percentile clip can be 1e-17 to 1e-13 s^-1, four orders of
 *  magnitude where linear would spend 99.99% of the slider on the top
 *  decade alone. Diverging variables are excluded outright: their range is
 *  symmetric about zero, where log has no meaning. */
const LOG_SCALE_MIN_RATIO = 100;

/**
 * A single-globe panel -- unlike ClimateUI, there is no multi-instance tiling
 * here (see docs/plans/deformation-viewer.md: multi-globe compare was
 * explicitly out of scope for v1), so this skips the whole
 * setRect()/Rect-based reflow ClimateUI needs and just anchors everything to
 * a fixed screen corner, the way a single always-was-single panel would.
 *
 * Layer switches which Model is loaded (Deformation vs Age & Heat Flux, see
 * CONTEXT.md's Layer entry). The age slider is Deformation-only: Age & Heat
 * Flux is assumed present-day with no reconstruction control at all (see
 * DeformationInstance.setLayer()), so this hides the slider entirely on that
 * Layer rather than leaving a control with nothing left for it to do.
 */
export class DeformationUI {
  readonly gui: GUI;
  ...
  private noDataStyleCtrl: Controller;
  /** The Variable rebuildClipControls() last built controllers for -- so
   *  toggling the log-scale checkbox (which doesn't go through
   *  setVariable()) knows what range/units to rebuild against. */
  private currentVariable: VariableInfo | null = null;
  /** Proxy object the log-mode sliders bind to (lil-gui binds directly to an
   *  object property, and the log-space number is not `state.clipMin/Max`
   *  itself -- those stay physical everywhere else in this class). */
  ...
  constructor(
    private state: DeformationViewState,
    private cb: DeformationUICallbacks,
  ) {
    this.gui = new GUI({ title: 'Geode Deformation' });
    // Which pipeline run/reconstruction (Muller2019, Cao2024, ...) is
    // driving both Layers below -- populated once the archive's available
    // reconstructions are known, see setReconstructions().
    this.reconstructionCtrl = this.gui
      .add(this.state, 'reconstruction', {})
      .name('reconstruction')
      .onChange((v: string) => cb.onReconstruction(v));
    this.layerCtrl = this.gui
      .add(this.state, 'layer', { Deformation: 'deformation', 'Age & Heat Flux': 'ageHeatflux' })
      .name('layer')
      .onChange((v: DeformationLayer) => cb.onLayer(v));
    this.variableCtrl = this.gui
      .add(this.state, 'variable', {})
      .name('variable')
      .onChange((v: string) => cb.onVariable(v));
    this.ageCtrl = this.gui.add(this.state, 'age', 0, 240, 1)
      .name('age (Ma)')
      .onChange((v: number) => cb.onAge(v));

    // Its own folder so clipMinCtrl/clipMaxCtrl can be destroyed and
    // rebuilt (different min/max/step per Variable, a different bound
    // object entirely in log mode) without reordering anything else in the
    // panel -- see rebuildClipControls().
    ...
    // See ADR-0005: unlike the mantle/climate viewers' fixed no-data grey,
    // this field is ~92% no-data by construction, so how absence is drawn
    // is left to the viewer rather than baked into the manifest.
  ...
  /** Populate the reconstruction dropdown -- called once at boot with
   *  whichever runs the archive actually has both Models for (see
   *  main.ts's RECONSTRUCTIONS filter). */
  setReconstructions(options: Array<{ id: string; label: string }>): void {
    const choices: Record<string, string> = {};
    for (const o of options) choices[o.label] = o.id;
    this.reconstructionCtrl.options(choices);
  }
  ...
  /** Age & Heat Flux has no reconstruction control at all (see
   *  DeformationInstance.setLayer()) -- hidden, not disabled, since a
   *  greyed-out slider with nothing left for it to do is still a dead
   *  control asking to be read as a bug. */
  setAgeControlVisible(visible: boolean): void {
    if (visible) this.ageCtrl.show(); else this.ageCtrl.hide();
  }
  ...
  /** (Re)build clipMinCtrl/clipMaxCtrl for `v`, in whichever of log/linear
   *  space `state.logScale` currently asks for -- called from setVariable()
   *  (a new Variable) and from the log-scale checkbox's own onChange (same
   *  Variable, different representation). Destroy-and-recreate rather than
   *  reusing one pair of controllers because log mode binds to a different
   *  object (`logProxy`) entirely, not just a different min/max/step on the
   *  same one. */
  private rebuildClipControls(v: VariableInfo): void {
    ...
    if (this.state.logScale && canLog) {
      ...
      // clampClipOrder works in PHYSICAL space (state.clipMin/Max); a
      // crossed pair here corrects there first, then the sibling's own
      // log-space proxy value is re-derived from the corrected physical
      // number before that controller's display is refreshed.
  ...
  /** Three significant figures for a linear tick; a tiny magnitude (a strain
   *  rate can be ~1e-14) needs exponential notation or it prints as "0". */
