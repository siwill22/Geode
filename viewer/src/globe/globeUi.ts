...
/** The fixed menu a recipe's `ui.tools` can draw from -- see
 *  generator/validateRecipe.mjs, which rejects any other string, and
 *  generated/config.ts, whose GlobeConfig.tools is typed against this same
 *  union so a typo is a compile error in a generated repo, not a silent
 *  no-op. */
...
export class GlobeUI {
  ...
  constructor(
    ...
    if (tools.includes('query-point')) {
      ...
      Object.assign(this.queryPanel.style, { top: '12px', right: '12px' });
   ...
   *  pointValue(), which reads straight from the currently-displayed
   *  frame's own bytes (no extra fetch), so this always reflects whatever
   *  age/variable is on screen right now. No-op if `query-point` wasn't in
   *  the recipe's tool list (queryPanel is null). */
